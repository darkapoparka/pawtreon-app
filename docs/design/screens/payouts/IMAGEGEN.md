# Image brief: Funding and payouts
...
Screen: `payouts`; initial state: `not-ready`; surface: planned route. Compose one readable first viewport at 390x844 browser-content proportions. Other states (pending-verification, ready, held, failed, permission-denied, offline) require separate images when applicable.
...
Keep the contract's defined inputs, actions, organizer identity and access rules. Initial state: Explain prerequisites; no fabricated balance or active payout action. Deeper content scrolls; do not shrink text to fit it all. Use clearly labeled illustrative concept data, never invented real donations or verified status.
...
Import and inspect the actual image before recording `concepts/mobile-390.not-ready.v1.png` as present. Compare with accepted homepage/detail components. A PNG does not prove working interactions or finish the route.
