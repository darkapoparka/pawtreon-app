# Image brief: Saved cases and campaigns
...
Screen: `saved`; initial state: `default`; surface: planned route. Compose one readable first viewport at 390x844 browser-content proportions. Other states (loading, empty, error, offline, permission-denied) require separate images when applicable.
...
1. Cases/campaigns tabs
2. Saved cards with current status
3. Remove saved item without nested card buttons
4. Useful empty state with browse link
