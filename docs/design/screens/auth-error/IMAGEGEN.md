# Image brief: This sign-in link expired
...
Screen: `auth-error`; initial state: `expired`; surface: planned route. Compose one readable first viewport at 390x844 browser-content proportions. Other states (invalid-link, error, offline) require separate images when applicable.
...
Keep the contract's defined inputs, actions, organizer identity and access rules. Initial state: Explain expiry and the relevant reissue/reverification action. Deeper content scrolls; do not shrink text to fit it all. Use clearly labeled illustrative concept data, never invented real donations or verified status.
...
Import and inspect the actual image before recording `concepts/mobile-390.expired.v1.png` as present. Compare with accepted homepage/detail components. A PNG does not prove working interactions or finish the route.
